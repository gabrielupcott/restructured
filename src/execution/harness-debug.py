...
def run_one():
    ...
    if finish is not None:
        import sys as _s
        print("finish=%r out=%r" % (finish, type(out)), file=_s.stderr)
        out = finish(out)
        print("finish ok, len=%r" % (len(out) if hasattr(out, "__len__") else "scalar",), file=_s.stderr)
    try:
